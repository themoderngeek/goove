...
- **Trailing newline in `parseStatus`.** Step 3 of Task 3 swaps the leading `TrimSpace` for `TrimRight(..., "\n")`. This is load-bearing: AppleScript emits a trailing newline after the last field, and the new last field (playlist name) may legitimately be the empty string. The previous `TrimSpace` would fold trailing empty lines into nothing, leaving 9 lines instead of 10 and triggering an `ErrUnavailable`.
